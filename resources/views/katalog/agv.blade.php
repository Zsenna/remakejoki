...
<body>
    ...
    <div id="outcon" class="container-fluid">
        <div class="container">
            @foreach ($agv as $a)
            <a href="{{url('Product-agv/' .$a->id)}}" class="box">
                <img src="{{$a->itemImg}}" class="img-fluid" alt="Image Error">
                <h4 class="card-title" style="color: white;">{{$a->itemText}}</h4>
                <p style="color: white;">{{Strip_tags($a->itemText)}}</p>
            </a>
            @endforeach
        </div>
    </div>
