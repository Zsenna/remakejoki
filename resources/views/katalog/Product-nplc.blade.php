...
<body>
    ...
    $nplc =
        DB::select('SELECT itemImg, itemText, itemDesc, itemSpec, id  FROM product WHERE itemTo = "nplc"');
    ?>
    @foreach ($nplc as $np)
    <div class="container" style="margin-top: 2em;">
        <div class="row">
            <div class="col-5">
                <img alt="" src="{{$np->itemImg}}" style=" height:auto; width:100%;box-shadow: 0em 0.063em 0.5em 0em #b6bec6;" />
            </div>
            <div class="col" style="text-align: left;">
                <h1>{{$np->itemText}}</h1>
                <ul style="font-size: 24px;">
                    <li>{{Strip_tags($np->itemSpec)}}</li>
                </ul>
            </div>
            <div class="container" style="margin-top: 2em;box-shadow: 0em 0.063em 0.5em 0em #b6bec6;">
                <h2 style="text-align: left;">Keterangan :</h2>
                <p style="text-align: justify;">{{Strip_tags($np->itemDesc)}}</p>
            </div>
        </div>
    </div>
    @endforeach
