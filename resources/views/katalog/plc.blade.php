...
<body>
    ...
    <div id="outcon" class="container-fluid">
        <div class="container">
            @foreach ($plc as $p)
            <a href="{{url('Product-plc/' .$p->id)}}" class="box">
                <img src="{{$p->itemImg}}" class="img-fluid" alt="Image Error">
                <h4 class="card-title" style="color: white;">{{$p->itemText}}</h4>
                <p style="color: white;">{{Strip_tags($p->itemText)}}</p>
            </a>
            @endforeach
        </div>
    </div>
